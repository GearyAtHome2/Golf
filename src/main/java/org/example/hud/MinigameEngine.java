...
public class MinigameEngine {
    public static class Zone {
        ...
        public Color color;
        public float center;
        public float widthRatio; // Ratio relative to the parent (Green)
    ...
    public MinigameResult calculateResult(float needlePos) {
        ...
        // Calculate all nested widths first to ensure collisions match the UI visuals
        ...
        // 1. Check Sweet Spots (Smallest to Largest / Last to First)
        for (int i = sweetSpots.size - 1; i >= 0; i--) {
            ...
            if (Math.abs(needlePos - z.center) <= zoneHalfW) {
                setupResult(result, z.powerMod, z.label);
                return result;
            }
        }
        ...
        if (Math.abs(needlePos - greenCenter) <= greenHalfW) {
            setupResult(result, 1.00f, GOOD);
            return result;
        }

        // 3. Penalty Zones (Red/Orange) and Misses
        ...
        if (distFromEdge <= orangeThreshold) {
            result.rating = MinigameResult.Rating.MEH;
            result.powerMod = 0.95f;
            result.accuracy = side * 0.15f;
        } else if (distFromEdge <= redThreshold) {
            result.rating = MinigameResult.Rating.WANK;
            result.powerMod = 0.85f;
            result.accuracy = side * 0.35f;
        } else {
            result.rating = MinigameResult.Rating.SHIT;
    ...
    private void setupResult(MinigameResult res, float power, MinigameResult.Rating rating) {
        res.powerMod = power;
        res.rating = rating;
        res.accuracy = 0;
    }
