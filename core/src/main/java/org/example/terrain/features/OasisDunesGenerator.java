...
/**
 * Archetype generator for OASIS_DUNES.
 *
 * v1 behaviour:
 ...
 *   3. Raises 1–2 Gaussian dune mounds away from the oasis and outside the
 *      green exclusion zone, so the flag is never on a steep dune slope.
 ...
 */
public class OasisDunesGenerator {
    ...
    private static final float GREEN_EXCLUSION    = 60.0f;  // min tile-distance from green centre → no dune centre
    private static final float OASIS_GREEN_EXCL  = 80.0f;  // min tile-distance from green centre → no oasis centre (larger to account for bowl radius)
    ...
    public OasisDunesGenerator(LevelData data) {
        this.data = data;
    ...
    public void generateOasisDunes(Terrain.TerrainType[][] map, float[][] heights, int greenCenterX, int greenCenterZ) {
        ...
        // Skip candidates too close to the green so the oasis bowl cannot flood into it.
        int   lowestX = SIZE_X / 2, lowestZ = (zMin + zMax) / 2;
        float lowestH = Float.MAX_VALUE;
        float oasisExclSq = OASIS_GREEN_EXCL * OASIS_GREEN_EXCL;
        for (int x = xMin; x < xMax; x++) {
            for (int z = zMin; z < zMax; z++) {
                float gdx = x - greenCenterX, gdz = z - greenCenterZ;
                if (gdx * gdx + gdz * gdz < oasisExclSq) continue;
        ...
        // ── 3. Raise 1–2 dune mounds away from the oasis and the green ───────
        ...
        for (int d = 0; d < numDunes; d++) {
            ...
            for (int attempt = 0; attempt < 15; attempt++) {
                ...
                float gdx = cx - greenCenterX, gdz = cz - greenCenterZ;
                if (Math.sqrt(ddx * ddx + ddz * ddz) >= minSep &&
                    Math.sqrt(gdx * gdx + gdz * gdz) >= GREEN_EXCLUSION) {
