...
public class SessionManager {
    ...
    public SessionManager(GameConfig config) {
        this.config = config;
        // Drain before loadSession so a finished file isn't silently deleted without upload.
        this.finishedStandard18PendingUpload = SessionPersistence.drainFinishedStandard18();
    ...
    public void setActive(GameSession session) {
        this.activeSession = session;
        if (session != null) {
            session.setSaveCallback(() -> SessionPersistence.saveSession(session, uid));
        }
    }
    ...
    public GameSession drainPendingStandard18Upload() {
        GameSession s = finishedStandard18PendingUpload;
        finishedStandard18PendingUpload = null;
        return s;
    }
