...
public class SessionPersistence {
    ...
    /**
     * If a finished STANDARD_18 save exists on disk, deletes it and returns the session
     * so the caller can upload it. Returns null if there is no finished session.
     * Must be called before loadSession(STANDARD_18) to avoid double-delete.
     */
    public static GameSession drainFinishedStandard18() {
        FileHandle file = Gdx.files.local(STANDARD_FILE);
        if (!file.exists()) return null;
        try {
            GameSession session = json.fromJson(GameSession.class, file.readString());
            ...
            Gdx.app.error("SessionPersistence", "drainFinishedStandard18 parse error", e);
        }
        return null;
    }
