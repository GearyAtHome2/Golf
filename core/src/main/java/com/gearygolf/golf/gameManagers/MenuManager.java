package com.gearygolf.golf.gameManagers;
...
public class MenuManager {
    ...
    private void handleBackNavigation() {
        switch (currentMenuState) {
            ...
            case SETTINGS -> { currentMenuState = MenuState.MAIN; menuSelection = 2; } // restore to SETTINGS position in main menu
    ...
    private void handleSettings(MenuHandler callback) {
        // SOUND(0) INSTRUCTIONS(1) PROFILE(2) BACK(3)
        switch (menuSelection) {
            case 0 -> callback.onShowSoundSettings();
            case 1 -> callback.onShowInstructions();
            case 2 -> callback.onShowProfile();
            case 3 -> { currentMenuState = MenuState.MAIN; menuSelection = 2; }
        }
    }
