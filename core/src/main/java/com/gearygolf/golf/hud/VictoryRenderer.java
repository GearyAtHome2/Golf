...
import com.gearygolf.golf.ScoreShout;
...
public class VictoryRenderer {
    ...
    public void render(SpriteBatch batch, ShapeRenderer shapeRenderer, BitmapFont font, Viewport viewport, int shots, LevelData levelData, GameSession session, boolean uploadDone) {
        ...
        if (session != null) {
            if (isOneHoleMode(session)) {
                ...
                batch.end();
                renderCompetitiveResults(batch, shapeRenderer, font, viewport, session, centerX, screenH * 0.45f, shout, shoutColor, subText, uploadDone);
                batch.begin();
            }
    ...
    private void renderCompetitiveResults(SpriteBatch batch, ShapeRenderer shapeRenderer, BitmapFont font, Viewport viewport,
                                          ...
                                          String shout, Color shoutColor, String subText, boolean uploadDone) {
        ...
        // Difficulty indicator — top-right corner of modal, small and dimmed
        ...
        float diffPadX = viewport.getWorldWidth()  * 0.012f;
        float diffPadY = viewport.getWorldHeight() * 0.018f;
        font.draw(batch, diffText, (centerX + boxWidth / 2f) - layout.width - diffPadX, modalTop - diffPadY);
        ...
        if (!Platform.isAndroid()) {
            ...
            if (session.isFinished()) {
                if (session.getMode() == GameSession.GameMode.DAILY_18 || session.getMode() == GameSession.GameMode.DAILY_9 || isOneHoleMode(session)) {
                    ...
                    prompt = uploadDone
                        ? "Score uploaded!  [M] Main Menu"
                        : "COURSE COMPLETE! [U] Upload Score  [M] Main Menu";
                } else {
