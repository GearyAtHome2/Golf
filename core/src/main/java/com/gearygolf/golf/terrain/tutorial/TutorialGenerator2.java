...
/**
 * Hardcoded par-4 tutorial hole (~700 yards) for the second tutorial level.
 * Teaches: spindicator placement (lower-right for loft + fade) and approach shot club selection.
 * Wide fairway, one fairway bunker right of centre. Wind: (3, 0, 5.2) — ~6 m/s from back-right,
 * blowing left, so lower-right spin (fade + loft) correctly offsets the crosswind.
 */
public class TutorialGenerator2 implements ITerrainGenerator {

    private static final long  INTERNAL_SEED   = 43L;
    private static final int   FAIRWAY_HALF_W  = 30;   // wide — forgiving tee shot
    private static final float GREEN_RADIUS    = 18f;
    private static final float BUNKER_RADIUS   = 8f;
    private static final float TEE_ELEVATION   = 2f;
    private static final float WATER_LEVEL     = -1f;

    // Multi-wave noise constants (different seed from level 1)
    ...
    private static final float[] WO = {0.3f,  1.5f,  0.8f,  2.4f,  1.1f};
    ...
    public void generate(Terrain.TerrainType[][] map, float[][] heights,
        ...
        // 1. Base height field — gentle uphill from tee to mid-fairway then flat.
        for (int x = 0; x < SX; x++) {
            for (int z = 0; z < SZ; z++) {
                float t = MathUtils.clamp((z - teeGZ) / (float)(greenGZ - teeGZ), 0f, 1f);
                // Rises slightly then levels off — gives a links par-4 feel.
                float rise = (t < 0.4f) ? t * 0.8f : 0.32f;
                heights[x][z] = 2.0f + rise + noise(x, z) * 1.4f;
                map[x][z] = Terrain.TerrainType.ROUGH;
            }
        }

        // 2. Fairway corridor — 20 cells wide (FAIRWAY_HALF_W = 10 each side).
        ...
        // 3. Green — flat circle at hole position.
        for (int x = 0; x < SX; x++) {
            for (int z = 0; z < SZ; z++) {
        ...
        // 5. Fairway bunker — right of centre at ~55% down the fairway (driver landing zone).
        //    Placed off the right edge of the fairway corridor to penalise a pushed tee shot.
        ...
        // 6. Greenside bunkers — left and right of green.
