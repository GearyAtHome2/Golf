package com.gearygolf.golf.terrain.objects;
...
public class Tree extends TerrainObject {
    ...
    public enum TreeScheme {
        ...
        // Low-saturation warm-grey bark; very desaturated dark-green needles
        BALD_CYPRESS(new Color(0.38f, 0.33f, 0.27f, 1f), new Color(0.50f, 0.44f, 0.36f, 1f), new Color(0.09f, 0.17f, 0.10f, 1f), new Color(0.14f, 0.25f, 0.14f, 1f));
    ...
    public Tree(float x, float y, float z, float th, float tr, float fr, TreeScheme scheme, java.util.Random rng, float mapRotation) {
        ...
        } else if (scheme == TreeScheme.BALD_CYPRESS) {
            // Same cone silhouette as FIR but 30% wider at the base
            float skirtMultiplier = 0.35f + (rng.nextFloat() * 0.31f);
            this.actualFoliageHeight = (th * skirtMultiplier) + (fr * 2);
            foliage = new ModelInstance(mb.createCone(fr * 3.25f, actualFoliageHeight, fr * 3.25f, 16, leafMat, attributes));
            foliage.transform.setToTranslation(x, y + th + (fr * 2) - (actualFoliageHeight / 2f), z);
    ...
    public boolean isInsideFoliage(Vector3 ballPos, float ballRadius) {
        ...
        if (scheme == TreeScheme.BALD_CYPRESS) {
            float foliageBottom = pos.y + tH + (fR * 2) - actualFoliageHeight;
            float foliageTop = pos.y + tH + (fR * 2);
            ...
            float heightPercent = 1.0f - ((ballPos.y - foliageBottom) / actualFoliageHeight);
            float coneRadiusAtHeight = (fR * 3.25f / 2f) * heightPercent;
