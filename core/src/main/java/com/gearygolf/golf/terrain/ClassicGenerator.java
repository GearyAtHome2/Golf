...
public class ClassicGenerator implements ITerrainGenerator {
    ...
    private void finalizePositionsAndTrees(Terrain.TerrainType[][] map, float[][] heights, Vector3 teeP, Vector3 holeP, List<Tree> trees, List<Monolith> monoliths, int gX, int gZ, float water, ArchetypeFlags flags) {
        ...
        float randomAngle = rng.nextFloat() * MathUtils.PI * 2, randomDist = rng.nextFloat() * (data.getGreenRadius() * 0.7f);
        ...
        if (flags.isVineyards) {
            ...
            generateRandomTrees(map, heights, trees, teeZ, teeX, water, flags.isCliffMap, gX, gZ);
        }
    }
    ...
    private void generateRandomTrees(Terrain.TerrainType[][] map, float[][] heights, List<Tree> trees, int teeZ, int teeX, float water, boolean isCliff, int gX, int gZ) {
        int SIZE_X = map.length, SIZE_Z = map[0].length;
        float cliffDelta = Math.abs(data.getTeeHeight() - data.getGreenHeight());
        ...
        boolean isBushWorld     = data.getArchetype() == LevelData.Archetype.BUSH_WORLD;
        boolean isCypressBayou  = data.getArchetype() == LevelData.Archetype.CYPRESS_BAYOU;
        ...
        for (int i = 0; i < treeCount; i++) {
            ...
            if (isCypressBayou) {
                // Bayou/mangrove: allow trees rooted in MUD or standing in shallow water,
                // but never inside the water-gap crossing zones.
                if (cypressBayouGenerator.getGapCells().contains(
                        CypressBayouGenerator.packCell(tx, tz))) continue;
                Terrain.TerrainType t = map[tx][tz];
                boolean validType = t == Terrain.TerrainType.ROUGH
            ...
            if (isBushWorld) {
                // Density falls off with distance from the green: full density at dist=0,
                // ~10% at dist=200. A ±30-unit jitter on the effective distance keeps
                // the boundary organic rather than a hard ring.
                float distToGreen = Vector2.dst(tx, tz, gX, gZ);
                float jitteredDist = distToGreen + (rng.nextFloat() - 0.5f) * 60f;
                float bushFalloff = MathUtils.clamp(0.1f + 0.9f * Math.max(0f, 1f - jitteredDist / 200f), 0.1f, 1.0f);
                treeChance *= bushFalloff;
            }
