...
public class BeachBluffsGenerator {
    ...
    public void generateBeachBLuffs(Terrain.TerrainType[][] map, float[][] heights, int gX, int gZ, float water) {
        ...
        // Guarantee at least one bluff within the first 200 Z-units near the tee.
        // The main loop only samples from SZ*0.2f onwards, leaving the tee-side zone empty.
        float nearTeeMax = Math.min(200f, SZ * 0.18f);
        boolean hasNearTeeBluff = false;
        for (int i = 2; i < points.size(); i++) {
            if (points.get(i).z < nearTeeMax) { hasNearTeeBluff = true; break; }
        }
        if (!hasNearTeeBluff) {
            int nearAttempts = 0;
            while (nearAttempts++ < 300) {
                float z = MathUtils.lerp(SZ * 0.08f, nearTeeMax, rng.nextFloat());
                float x = (SX * 0.15f) + rng.nextFloat() * (SX * 0.7f);
                if (isSpaceForBluff(x, z, points, 55.0f)) {
                    float t = MathUtils.clamp((z - (SZ * 0.05f)) / (SZ * 0.87f), 0, 1);
                    points.add(new Vector3(x, MathUtils.lerp(data.getTeeHeight(), data.getGreenHeight(), t) + (rng.nextFloat() - 0.5f) * 5f, z));
                    break;
                }
